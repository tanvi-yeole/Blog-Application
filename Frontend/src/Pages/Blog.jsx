...
const BlogPost = () => {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/getPost/${id}`)
      .then(res => {
  ...
  const handleLike = () => {
    ...
    // axios.post(`http://localhost:4000/likePost/${id}`, { liked: !liked });
  };
    ...
    <Container maxWidth="md" sx={{ my: 5 }}>
      <Paper elevation={3} sx={{ p: 3 }}>
        ...
        <Box display="flex" justifyContent="center" my={2}>
          <img src={`${import.meta.env.VITE_API_URL}/${posts.data.cover}`} alt="Banner" style={{ maxWidth: '100%', borderRadius: 8 }} />
        </Box>
